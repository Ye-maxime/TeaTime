import {
    LOADED_STORES, FETCH_STORES, SHOW_STORE, CLICK_STORE,
} from '../actions/stores';
...
export default function reducerStores(state = STORES_DEFAULT_STATE, action) {
    ...
    case SHOW_STORE: {
        ...
        const storeFind = state.items.find((store) => store.latitude === action.store.lat && store.longitude === action.store.lng)
